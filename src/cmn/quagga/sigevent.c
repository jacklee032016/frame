...
quagga_signal_timer (struct thread *t)
{
  ...
  sigm = TASK_ARG (t);
  sigm->t = sysThreadAddTimer (sigm->t->master, quagga_signal_timer, &sigmaster,
...
#ifdef REG_INDEX
# ifdef HAVE_UCONTBASE_T_UC_MCONTBASE_GREGS
#  define REGS gregs[REG_INDEX]
# elif defined(HAVE_UCONTBASE_T_UC_MCONTBASE_UC_REGS)
#  define REGS uc_regs->gregs[REG_INDEX]
# endif /* HAVE_UCONTBASE_T_UC_MCONTBASE_GREGS */
#endif /* REG_INDEX */

#ifdef REGS
  if (context)
    return (void *)(((ucontext_t *)context)->uc_mcontext.REGS);
#elif defined(HAVE_UCONTBASE_T_UC_MCONTBASE_REGS__NIP) 
	      ...
	      (oact.sa_handler == SIG_DFL))
	    {
	      ...
	      if (sigaction(sigmap[i].sigs[j],&act,NULL) < 0)
	        zlog_warn("Unable to set signal handler for signal %d: %s",
			  sigmap[i].sigs[j],cmnSafeStrError(errno));
...
#ifdef SIGEVENT_SCHEDULE_THREAD  
  sigmaster.t = 
    sysThreadAddTimer (m, quagga_signal_timer, &sigmaster, 
                      QUAGGA_SIGNAL_TIMER_INTERVAL);
#endif /* SIGEVENT_SCHEDULE_THREAD */
}

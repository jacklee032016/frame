/*
* common pthread implementation in Windows
*/
...
{
	HRESULT hr;
	hr = SetThreadDescription(thId, threadName);
	if (SUCCEEDED(hr))
	{   
	}
	else
	{
		printf("Set Thread Description Failed");
		return -1;
	}
	return 0;
}
...
int pthread_getname_np(pthread_t thId, char *threadName, int size)
{
	...
	char		*_name;
	HRESULT hr = GetThreadDescription(thId, &_name);
	if (SUCCEEDED(hr))
	{   
		snprintf(threadName, size,  "%s", _name);
		LocalFree(_name);		
	}
	else
	{
		printf("Get Thread Description Failed");
		ret = -1;
	}
...
int pthreadd_mutex_trylock(pthread_mutex_t* mutex)
{

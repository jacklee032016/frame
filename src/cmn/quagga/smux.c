...
smux_read (struct thread *t)
{
  ...
  if (len < 0)
    {
      zlog_warn ("Can't read all SMUX packet: %s", cmnSafeStrError (errno));
      close (sock);
      smux_sock = -1;
...
smux_connect (struct thread *t)
{
  ...
  if (ret < 0)
    {
      zlog_warn ("SMUX open message send failed: %s", cmnSafeStrError (errno));
      close (smux_sock);
      smux_sock = -1;
      if (++fail < SMUX_MAX_FAILURE)
	smux_event (SMUX_CONNECT, 0);
  ...
  if (ret < 0)
    {
      zlog_warn ("SMUX register message send failed: %s", cmnSafeStrError (errno));
      close (smux_sock);
      smux_sock = -1;
      if (++fail < SMUX_MAX_FAILURE)
	smux_event (SMUX_CONNECT, 0);
...
smux_event (enum smux_event event, int sock)
{
  switch (event)
    ...
    case SMUX_SCHEDULE:
      ...
      break;
    case SMUX_CONNECT:
      smux_connect_thread = sysThreadAddTimer (smux_master, smux_connect, NULL, 10);
      break;
    case SMUX_READ:

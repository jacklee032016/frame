...
/* Only build when BASE_HAS_SSL_SOCK is enabled and when the backend is
 * OpenSSL.
 */
#if defined(BASE_HAS_SSL_SOCK) && BASE_HAS_SSL_SOCK != 0 && \
    (BASE_SSL_SOCK_IMP == BASE_SSL_SOCK_IMP_OPENSSL)
...
{
    unsigned i, cid = 0;
    for (i=0; i<BASE_ARRAY_SIZE(nid_cid_map); ++i) {
...
{
    if ((cid == 0) || (cid > BASE_ARRAY_SIZE(nid_cid_map)))
	return 0;
 ...
 * since MacOSX 10.7).
 */
#if defined(BASE_DARWINOS) && BASE_DARWINOS==1
...
/**
 * Mapping from OpenSSL error codes to libBase error space.
 */
...
#define ERROR_LOG(msg, err) \
    BASE_STR_INFO("SSL", "%s (%s): Level: %d err: <%lu> <%s-%s-%s> len: %d", \
...
static void SSLLogErrors(char * action, int ret, int ssl_err, int len)
{
	...
	if (err2) {
	    ...
	    BASE_ERROR("BIO error, SSL_ERROR_SYSCALL (%s): "
	    		     "errno: <%d> <%s> len: %d", action, errno, errStr, len);
	} else {
	    /* ret!=0 & ret!=-1 & nothing on error stack - is this valid??? */
	    BASE_ERROR("SSL_ERROR_SYSCALL (%s) ret: %d len: %d", action, ret, len);
	}
	break;
    }
    case SSL_ERROR_SSL:
    ...
    default:
	BASE_ERROR("%lu [%s] (%s) ret: %d len: %d", ssl_err, ssl_err_str, action, ret, len);
	break;
    }
}


static bstatus_t GET_STATUS_FROM_SSL_ERR(unsigned long err)
{
    bstatus_t status;

    /* OpenSSL error range is much wider than  errno space, so
...
static bstatus_t STATUS_FROM_SSL_ERR(char *action, bssl_sock_t *ssock,
				       unsigned long err)
{
    int level = 0;
    int len = 0; //dummy
...
static bstatus_t STATUS_FROM_SSL_ERR2(char *action, bssl_sock_t *ssock,
...
static bstatus_t GET_SSL_STATUS(bssl_sock_t *ssock)
{
 ...
 */
static bstr_t ssl_strerror(bstatus_t status, 
			     char *buf, bsize_t bufsize)
{
    ...
    errstr.ptr = buf;
    errstr.slen = bansi_snprintf(buf, bufsize, 
                            ...
                            const buint8_t *src, bsize_t len)
{
    ...
    return (nwritten < (int)len)? GET_SSL_STATUS(cb->owner): BASE_SUCCESS;
}
...
/* Initialize OpenSSL */
static bstatus_t init_openssl(void)
{
    bstatus_t status;
    ...
    status = bregister_strerror(BASE_SSL_ERRNO_START, 
				  BASE_SSL_ERRNO_SPACE_SIZE, 
				  &ssl_strerror);
	...
	if (ssl_curves_num > BASE_ARRAY_SIZE(ssl_curves))
	    ssl_curves_num = BASE_ARRAY_SIZE(ssl_curves);

	for (i = 0; i < ssl_curves_num; i++) {
	    nid = SSL_get_shared_curve(ssl, i);

	    if (nid & TLSBASE_nid_unknown) {
		cname = "curve unknown";
		nid &= 0xFFFF;
	    } else {
...
{
    BASE_UNUSED_ARG(openssl_init_count);
}
...
{
    bssl_cert_t *cert = (bssl_cert_t*) user_data;

    BASE_UNUSED_ARG(rwflag);

    if(num < cert->privkey_pass.slen)
	return 0;
    
    bmemcpy(buf, cert->privkey_pass.ptr, cert->privkey_pass.slen);
...
static int verify_cb(int preverify_ok, X509_STORE_CTX *x509_ctx)
{
    ...
    case X509_V_ERR_ERROR_IN_CRL_NBASE_UPDATE_FIELD:
	ssock->verify_status |= BASE_SSL_CERT_ECRL_FAILURE;
	break;	
	...
	ssock->verify_status |= BASE_SSL_CERT_EINVALID_PURPOSE;
	break;

    case X509_V_ERR_CERT_CHAIN_TOO_LONG: /* not really used */
    case X509_V_ERR_PATH_LENGTH_EXCEEDED:
	ssock->verify_status |= BASE_SSL_CERT_ECHAIN_TOO_LONG;
	break;
     ...
     */
    if (BASE_FALSE == ssock->param.verify_peer)
	preverify_ok = 1;
...
static bstatus_t set_cipher_list(bssl_sock_t *ssock);
/* Setting SSL sock curves list */
static bstatus_t set_curves_list(bssl_sock_t *ssock);
/* Setting sigalgs list */
static bstatus_t set_sigalgs(bssl_sock_t *ssock);
/* Setting entropy for rng */
static void set_entropy(bssl_sock_t *ssock);


static bssl_sock_t *ssl_alloc(bpool_t *pool)
{
    return (bssl_sock_t *)BASE_POOL_ZALLOC_T(pool, ossl_sock_t);
}
...
static bstatus_t ssl_create(bssl_sock_t *ssock)
{
	...
	ssock->param.proto = BASE_SSL_SOCK_PROTO_SSL23;
    ...
    case BASE_SSL_SOCK_PROTO_TLS1:
	ssl_method = (SSL_METHOD*)TLSv1_method();
	break;
#ifndef OPENSSL_NO_SSL2
    case BASE_SSL_SOCK_PROTO_SSL2:
	ssl_method = (SSL_METHOD*)SSLv2_method();
	break;
#endif
#ifndef OPENSSL_NO_SSL3_METHOD
    case BASE_SSL_SOCK_PROTO_SSL3:
	...
	ssl_opt |= ((ssock->param.proto & BASE_SSL_SOCK_PROTO_SSL2)==0)?
		    SSL_OP_NO_SSLv2:0;
#endif

#ifdef SSL_OP_NO_SSLv3
	/** Check if SSLv3 is enabled */
	ssl_opt |= ((ssock->param.proto & BASE_SSL_SOCK_PROTO_SSL3)==0)?
		    SSL_OP_NO_SSLv3:0;
#endif

#ifdef SSL_OP_NO_TLSv1
	/** Check if TLSv1 is enabled */
	ssl_opt |= ((ssock->param.proto & BASE_SSL_SOCK_PROTO_TLS1)==0)?
		    SSL_OP_NO_TLSv1:0;
#endif

#ifdef SSL_OP_NO_TLSv1_1
	/** Check if TLSv1_1 is enabled */
	ssl_opt |= ((ssock->param.proto & BASE_SSL_SOCK_PROTO_TLS1_1)==0)?
		    SSL_OP_NO_TLSv1_1:0;
#endif

#ifdef SSL_OP_NO_TLSv1_2
	/** Check if TLSv1_2 is enabled */
	ssl_opt |= ((ssock->param.proto & BASE_SSL_SOCK_PROTO_TLS1_2)==0)?
		    SSL_OP_NO_TLSv1_2:0;
	...
	if (cert->privkey_file.slen) {
		...
		if (bio != NULL) {
			...
			if (SSL_CTX_set_tmp_dh(ctx, dh)) {
			    ...
			    BASE_STR_INFO(ssock->pool->objName, "SSL DH initialized, PFS cipher-suites enabled"));
			}
			DH_free(dh);
		    }
		    BIO_free(bio);
	...
	if (cert && cert->cert_file.slen) {
	    const bstr_t RSA = {"_rsa.", 5};
	    p = bstrstr(&cert->cert_file, &RSA);
	...
	if (p) {
	    ...
	    for (i = 0; i < BASE_ARRAY_SIZE(cert_types); ++i) {
		int err;

		bmemcpy(p, cert_types[i], CERT_TYPE_LEN);
		if (!bfile_exists(cf))
		    continue;
		...
		if (err == 1)
		    err = SSL_CTX_use_PrivateKey_file(ctx, cf,
						      SSL_FILETYPE_PEM);
		if (err == 1) {
		    BASE_STR_INFO(ssock->pool->objName,"Additional certificate '%s' loaded.", cf);
		} else {
		    BASE_PERROR(1,(ssock->pool->objName, GET_SSL_STATUS(ssock),
	...
	if (SSL_CTX_ctrl(ctx, SSL_CTRL_SET_ECDH_AUTO, 1, NULL)) {
	    BASE_STR_INFO(ssock->pool->objName, "SSL ECDH initialized (automatic), faster PFS ciphers enabled"));
    #if !defined(OPENSSL_NO_ECDH) && OPENSSL_VERSION_NUMBER >= 0x10000000L && \
	OPENSSL_VERSION_NUMBER < 0x10100000L
	} else {
		...
		if (SSL_CTX_set_tmp_ecdh(ctx, ecdh)) {
		    BASE_STR_INFO(ssock->pool->objName, "SSL ECDH initialized (secp256r1), faster PFS cipher-suites enabled");
		}
		EC_KEY_free(ecdh);
	    }
    #endif
...
static bstatus_t set_cipher_list(bssl_sock_t *ssock)
{
	...
	ret = SSL_set_cipher_list(ossock->ossl_ssl, BASE_SSL_SOCK_OSSL_CIPHERS);
    	if (ret < 1) {
	    return GET_SSL_STATUS(ssock);
    	}    
    ...
    tmp_pool = bpool_create(ssock->pool->factory, "ciphpool", BUF_SIZE, 
			      BUF_SIZE/2 , NULL);
    if (!tmp_pool)
	return BASE_ENOMEM;

    buf = (char *)bpool_zalloc(tmp_pool, BUF_SIZE);
	...
	for (j = 0; j < sk_SSL_CIPHER_num(sk_cipher); ++j) {
	    ...
	    if (ssock->param.ciphers[i] == (bssl_cipher)
					   ((buint32_t)SSL_CIPHER_get_id(c) &
					   0x00FFFFFF))
	    {
		const char *c_name;
		...
		/* Check buffer size */
		if (cipher_list.slen + bansi_strlen(c_name) + 2 >
		    BUF_SIZE)
		{
		    bassert(!"Insufficient temporary buffer for cipher");
		    return BASE_ETOOMANY;
		}

		/* Add colon separator */
		if (cipher_list.slen)
		    bstrcat2(&cipher_list, ":");

		/* Add the cipher */
		bstrcat2(&cipher_list, c_name);
		break;
	    }
	}	
    }
...
static bstatus_t set_curves_list(bssl_sock_t *ssock)
{
    ...
    ossl_sock_t *ossock = (ossl_sock_t *)ssock;
    int ret;
    int curves[BASE_SSL_SOCK_MAX_CURVES];
    unsigned cnt;
...
static void set_entropy(bssl_sock_t *ssock)
{
	...
	case BASE_SSL_ENTROPY_EGD:
	    ret = RAND_egd(ssock->param.entropy_path.ptr);
	    break;
#endif
	case BASE_SSL_ENTROPY_RANDOM:
	    ret = RAND_load_file("/dev/random",255);
	    break;
	case BASE_SSL_ENTROPY_URANDOM:
	    ret = RAND_load_file("/dev/urandom",255);
	    break;
	case BASE_SSL_ENTROPY_FILE:
	    ret = RAND_load_file(ssock->param.entropy_path.ptr,255);
	    break;
	case BASE_SSL_ENTROPY_NONE:
	default:
	    break;
    }

    if (ret < 0) {
	BASE_STR_INFO(ssock->pool->objName, "SSL failed to reseed with entropy type %d [native err=%d]", ssock->param.entropy_type, ret));
    }
}

/* Parse OpenSSL ASN1_TIME to btime_val and GMT info */
static bbool_t parse_ossl_asn1_time(btime_val *tv, bbool_t *gmt,
				      const ASN1_TIME *tm)
{
...
static void get_cn_from_gen_name(const bstr_t *gen_name, bstr_t *cn)
{
    bstr_t CN_sign = {"/CN=", 4};
    char *p, *q;

    bbzero(cn, sizeof(bstr_t));
 ...
 */
static void get_cert_info(bpool_t *pool, bssl_cert_info *ci, X509 *x,
			  bbool_t get_pem)
{
    ...
    /* Get serial no */
    q = (const buint8_t*) M_ASN1_STRING_data(X509_get_serialNumber(x));
    len = M_ASN1_STRING_length(X509_get_serialNumber(x));
    if (len > sizeof(ci->serial_no)) 
	len = sizeof(ci->serial_no);
    bmemcpy(serial_no + sizeof(ci->serial_no) - len, q, len);

    /* Check if the contents need to be updated. */
    update_needed = bstrcmp2(&ci->issuer.info, buf) || 
	            bmemcmp(ci->serial_no, serial_no, sizeof(ci->serial_no));
    if (!update_needed)
	return;
    ...
    if (ci->version >= 3) {
	names = (GENERAL_NAMES*) X509_get_bd2i(x, NID_subject_alt_name,
						  NULL, NULL);
    }
    if (names) {
        unsigned i, cnt;
	...
	ci->subj_alt_name.entry = bpool_calloc(pool, cnt, 
	...
	if (!PEM_write_bio_X509(bio, x)) {
	    BASE_ERROR("Error retrieving raw certificate info");
	    ci->raw.ptr = NULL;
	    ci->raw.slen = 0;
	} else {
	    BIO_write(bio, "\0", 1);
	    BIO_get_mem_ptr(bio, &ptr);
	    bstrdup2(pool, &ci->raw, ptr->data);	
	}	
	BIO_free(bio);	    
    }	 
}
 ...
 */
static void ssl_update_remote_cert_chain_info(bpool_t *pool,
					      bssl_cert_info *ci,
					      STACK_OF(X509) *chain,
					      bbool_t get_pem)
{
    ...
    /* For now, get_pem has to be BASE_TRUE */
    bassert(get_pem);
    BASE_UNUSED_ARG(get_pem);

    ci->raw_chain.cert_raw = (bstr_t *)bpool_calloc(pool,
       				    			sk_X509_num(chain),
       				    			sizeof(bstr_t));
	...
	X509 *x = sk_X509_value(chain, i);
        ...
        if (!PEM_write_bio_X509(bio, x)) {
            ...
            BIO_write(bio, "\0", 1);
            BIO_get_mem_ptr(bio, &ptr);
            bstrdup2(pool, &ci->raw_chain.cert_raw[i], ptr->data );
        }
...
static void ssl_update_certs_info(bssl_sock_t *ssock)
{
    ...
    if (x) {
	get_cert_info(ssock->pool, &ssock->local_cert_info, x, BASE_FALSE);
    ...
    if (x) {
	get_cert_info(ssock->pool, &ssock->remote_cert_info, x, BASE_TRUE);
	/* Free peer's X509 */
	X509_free(x);
    } else {
...
{
#ifdef SSL_set_tlsbhost_name
    ossl_sock_t *ossock = (ossl_sock_t *)ssock;

    /* Set server name to connect */
    if (ssock->param.server_name.slen) {
	/* Server name is null terminated already */
	if (!SSL_set_tlsbhost_name(ossock->ossl_ssl, 
				      ssock->param.server_name.ptr))
	{
	    char err_str[BASE_ERR_MSG_SIZE];

	    ERR_error_string_n(ERR_get_error(), err_str, sizeof(err_str));
	    BASE_STR_INFO(ssock->pool->objName, "SSL_set_tlsbhost_name() failed: %s", err_str));
	}
    }
#endif
}
    ...
    *size = size_ = SSL_read(ossock->ossl_ssl, data, size_);
	...
	if (err != SSL_ERROR_NONE && err != SSL_ERROR_WANT_READ) {
		...
		status = STATUS_FROM_SSL_ERR2("Read", ssock, size_,
					      err, len);
		BASE_STR_INFO("SSL", "SSL_read() = -1, with "
				 "SSL_ERROR_SYSCALL, no SSL error, "
				 "and errno = 0 - skip BIO error");
		/* Ignore these errors */
	    } else {
		/* Reset SSL socket state, then return BASE_FALSE */
		status = STATUS_FROM_SSL_ERR2("Read", ssock, size_,
		        		      err, len);

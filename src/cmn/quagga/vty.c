...
vty_out (struct vty *vty, const char *format, ...)
{
  ...
  if (vty_shell (vty))
	...
	p = buf;
      ...
      cmnBufferPut (vty->obuf, (u_char *) p, len);
  ...
  if (write(vty->wfd, buf, len) < 0)
	...
	return -1;
      ...
      zlog_warn("%s: write failed to vty client fd %d, closing: %s",
		__func__, vty->fd, cmnSafeStrError(errno));
      cmnBufferReset(vty->obuf);
...
vty_new ()
{
  ...
  new->obuf = cmnBufferNew(0);	/* Use default buffer size. */
...
vty_write (struct vty *vty, const char *buf, size_t nbytes)
{
  ...
  cmnBufferPut (vty->obuf, buf, nbytes);
}
...
vty_flush (struct thread *thread)
{
  ...
  if ((vty->lines == 0) || (vty->width == 0) || (vty->height == 0))
    flushrc = cmnBufferFlushAvailable(vty->obuf, vty_sock);
...
vty_accept (struct thread *thread)
{
  ...
  if (vty_sock < 0)
    {
      zlog_warn ("can't accept vty socket : %s", cmnSafeStrError (errno));
      return -1;
    }
...
vty_serv_sock_family (const char* addr, unsigned short port, int family)
{
  ...
  if(naddr)
	...
	break;
      case 0:
        zlog_err("error translating address %s: %s",addr,cmnSafeStrError(errno));
	naddr=NULL;
    }
...
vty_serv_un (const char *path)
{
  ...
  if (sock < 0)
    {
      zlog_err("Cannot create unix stream socket: %s", cmnSafeStrError(errno));
      return;
    }
  ...
  if (ret < 0)
    {
      zlog_err("Cannot bind path %s: %s", path, cmnSafeStrError(errno));
      close (sock);	/* Avoid sd leak. */
      return;
    }
  ...
  if (ret < 0)
    {
      zlog_err("listen(fd %d) failed: %s", sock, cmnSafeStrError(errno));
      close (sock);	/* Avoid sd leak. */
      return;
    }
  ...
  if (set_nonblocking(sock) < 0)
    {
      zlog_warn ("vtysh_accept: could not set vty socket %d to non-blocking,"
                 " %s, closing", sock, cmnSafeStrError (errno));
      close (sock);
      return -1;
    }
...
{
  switch (cmnBufferFlushAvailable(vty->obuf, vty->wfd))
    {
    case BUFFER_PENDING:
...
vtysh_read (struct thread *thread)
{
  ...
  if ((nbytes = read (sock, buf, VTY_READ_BUFSIZ)) <= 0)
      ...
      if (nbytes < 0)
	{
	  ...
	  zlog_warn("%s: read failed on vtysh client fd %d, closing: %s",
		    __func__, sock, cmnSafeStrError(errno));
	}
      cmnBufferReset(vty->obuf);
      vty_close (vty);
#ifdef VTYSH_DEBUG
  ...
  for (p = buf; p < buf+nbytes; p++)
      ...
      if (*p == '\0')
	{
	  ...
	  header[3] = ret;
	  cmnBufferPut(vty->obuf, header, 4);
...
vty_close (struct vty *vty)
{
  ...
  /* Flush buffer. */
  cmnBufferFlushAll (vty->obuf, vty->wfd);

  /* Free input buffer. */
  cmnBufferFree (vty->obuf);
...
vty_read_file (FILE *confp)
{
  ...
  cmnBufferFlushAll (vty->obuf, vty->fd);
  ...
  if (config_file != NULL)
      ...
      if (confp == NULL)
        {
          fprintf (stderr, "%s: failed to open configuration file %s: %s\n",
                   __func__, fullpath, cmnSafeStrError (errno));
  ...
  else
    {
      ...
      if (confp == NULL)
        {
          fprintf (stderr, "%s: failed to open configuration file %s: %s\n",
                   __func__, config_default_dir, cmnSafeStrError (errno));

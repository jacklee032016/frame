...
sockunion_socket (const union sockunion *su)
{
  ...
  if (sock < 0)
    {
      zlog (NULL, LOG_WARNING, "Can't make socket : %s", cmnSafeStrError (errno));
      return -1;
    }
		...
		union sockunion *su_addr)
{
  ...
  if (ret < 0)
    zlog (NULL, LOG_WARNING, "can't bind socket : %s", cmnSafeStrError (errno));
  ...
  if (family == AF_INET)
    {
	  ...
	  zlog (NULL, LOG_WARNING,
		...
		minttl, sock, cmnSafeStrError (errno));
      return ret;
    }
#endif /* IP_MINTTL */

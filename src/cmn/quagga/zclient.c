...
zclient_flush_data(struct thread *thread)
{
  ...
  if (zclient->sock < 0)
    return -1;
  switch (cmnBufferFlushAvailable(zclient->wb, zclient->sock))
    {
    case BUFFER_ERROR:
      zlog_warn("%s: cmnBufferFlushAvailable failed on zclient fd %d, closing",
...
zclient_send_message(struct zclient *zclient)
{
  if (zclient->sock < 0)
    return -1;
  switch (cmnBufferWrite(zclient->wb, zclient->sock, STREAM_DATA(zclient->obuf),

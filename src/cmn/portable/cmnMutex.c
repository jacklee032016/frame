...
{
	cmn_mutex_t *mut = (cmn_mutex_t *) MALLOC (sizeof (cmn_mutex_t));
...
{
	cmn_sem_t *sem = (cmn_sem_t *) MALLOC (sizeof (cmn_sem_t));
...
cmn_sem_t *cmn_sem_init (unsigned int value)
{
	...
	if (sem->semid == -1)
	{
		CMN_ERROR( "semget error : %s", strerror(errno) );
		FREE (sem);
		return NULL;
	}
	...
	if (i != 0)
	{
		CMN_ERROR( "semctl error : %s", strerror(errno) );
		FREE (sem);
		return NULL;
	}
...
{
	cmn_cond_t *cond = (cmn_cond_t *) MALLOC (sizeof (cmn_cond_t));

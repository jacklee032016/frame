...
typedef struct PjUwpSocketCallback
{
    ...
    void (*on_connect)(PjUwpSocket *s, bstatus_t status);
} PjUwpSocketCallback;
...
public:
    ...
    bsockaddr* GetLocalAddr() { return &local_addr; }
    bsockaddr* GetRemoteAddr() { return &remote_addr; }


    bstatus_t Bind(const bsockaddr_t *addr = NULL);
    bstatus_t Send(const void *buf, bssize_t *len);
    bstatus_t SendTo(const void *buf, bssize_t *len, const bsockaddr_t *to);
    bstatus_t Recv(void *buf, bssize_t *len);
    bstatus_t RecvFrom(void *buf, bssize_t *len, bsockaddr_t *from);
    bstatus_t Connect(const bsockaddr_t *addr);
    bstatus_t Listen();
    bstatus_t Accept(PjUwpSocket **new_sock);

    void (*on_read)(PjUwpSocket *s, int bytes_read);
    void (*on_write)(PjUwpSocket *s, int bytes_sent);
    void (*on_accept)(PjUwpSocket *s, bstatus_t status);
    void (*on_connect)(PjUwpSocket *s, bstatus_t status);

private:
    PjUwpSocket* CreateAcceptSocket(Windows::Networking::Sockets::StreamSocket^ stream_sock_);
    bstatus_t SendImp(const void *buf, bssize_t *len);
    ...
    bbool_t is_blocking;
    bbool_t has_pending_bind;
    bbool_t has_pending_send;
    bbool_t has_pending_recv;
    void *user_data;
...
inline bstatus_t wstr_addr_to_sockaddr(const wchar_t *waddr,
					 const wchar_t *wport,
					 bsockaddr_t *sockaddr)
{
#if 0
    char tmp_str_buf[BASE_INET6_ADDRSTRLEN+1];
    bassert(wcslen(waddr) < sizeof(tmp_str_buf));
    bunicode_to_ansi(waddr, wcslen(waddr), tmp_str_buf, sizeof(tmp_str_buf));
    bstr_t remote_host;
    bstrset(&remote_host, tmp_str_buf, bansi_strlen(tmp_str_buf));
    bsockaddr_parse(bAF_UNSPEC(), 0, &remote_host, (bsockaddr*)sockaddr);
    bsockaddr_set_port((bsockaddr*)sockaddr,  (buint16_t)_wtoi(wport));
    ...
    bstr_t remote_host;
    bstrset(&remote_host, tmp_str_buf, bansi_strlen(tmp_str_buf));
    bsockaddr *addr = (bsockaddr*)sockaddr;
    bbool_t got_addr = BASE_FALSE;

    if (binet_pton(BASE_AF_INET, &remote_host, &addr->ipv4.sin_addr)
	== BASE_SUCCESS)
    {
	addr->addr.sa_family = BASE_AF_INET;
	got_addr = BASE_TRUE;
    } else if (binet_pton(BASE_AF_INET6, &remote_host, &addr->ipv6.sin6_addr)
	== BASE_SUCCESS)
    {
    ...
    bsockaddr_set_port(addr, (buint16_t)_wtoi(wport));
    return BASE_SUCCESS;
}


inline bstatus_t sockaddr_to_hostname_port(const bsockaddr_t *sockaddr,
					     Windows::Networking::HostName ^&hostname,
					     int *port)
{
    char tmp[BASE_INET6_ADDRSTRLEN];
    wchar_t wtmp[BASE_INET6_ADDRSTRLEN];
    bsockaddr_print(sockaddr, tmp, BASE_INET6_ADDRSTRLEN, 0);
    bansi_to_unicode(tmp, bansi_strlen(tmp), wtmp,
	BASE_INET6_ADDRSTRLEN);
    hostname = ref new Windows::Networking::HostName(ref new Platform::String(wtmp));
    *port = bsockaddr_get_port(sockaddr);
